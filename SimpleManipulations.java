...
public class SimpleManipulations {
    ...
    public static PixelPicture border(PixelPicture pic, int borderWidth, Pixel borderColor) {
        ...
        Pixel[][] src = pic.getBitmap();
        Pixel[][] tgt = new Pixel[h + (2 * borderWidth)][w + (2 * borderWidth)]; // extend pic

        for (int col = 0; col < (h + (2 * borderWidth)); col++) {
            for (int row = 0; row < (w + (2 * borderWidth)); row++) {
                if ((col < borderWidth) || (row < borderWidth)) {
                    tgt[col][row] = borderColor;
                } else if ((col >= h + borderWidth) || (row >= w + borderWidth)) {
                    tgt[col][row] = borderColor;
                } else {
                    tgt[col][row] = Pixel.BLACK;
                }
            }
        }
        for (int col = 0; col < w; col++) {
    ...
    public static PixelPicture invertColors(PixelPicture pic) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = bmp[row][col];
                int newR = Math.abs(255 - p.getRed());
                int newG = Math.abs(255 - p.getGreen());
                int newB = Math.abs(255 - p.getBlue());
    ...
    public static PixelPicture grayScaleAverage(PixelPicture pic) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                int avg = (int) Math.round(total / 3.0);
                bmp[row][col] = new Pixel(avg, avg, avg);
            }
        }
        return new PixelPicture(bmp);
    ...
    public static PixelPicture scaleColors(PixelPicture pic,
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = bmp[row][col];
                int scaledR = (int) Math.round(p.getRed() * rfactor);
                int scaledG = (int) Math.round(p.getGreen() * gfactor);
                int scaledB = (int) Math.round(p.getBlue() * bfactor);

                bmp[row][col] = new Pixel(scaledR, scaledG, scaledB);
            }
        }
        return new PixelPicture(bmp);
    }
    ...
    public static PixelPicture alphaBlend(double alpha, PixelPicture pic, PixelPicture f) {
        ...
        for (int col = 0; col < w1; col++) {
            for (int row = 0; row < h1; row++) {
                ...
                int weightedR = (int) Math.round(
                        weightedAverage(alpha, p1.getRed(), p2.getRed()));
                int weightedG = (int) Math.round(
                        weightedAverage(alpha, p1.getGreen(), p2.getGreen()));
                int weightedB = (int) Math.round(
                        weightedAverage(alpha, p1.getBlue(), p2.getBlue()));

                bmp1[row][col] = new Pixel(weightedR, weightedG, weightedB);
            }
        }
